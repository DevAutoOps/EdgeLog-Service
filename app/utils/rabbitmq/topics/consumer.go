...
type consumer struct {
	...
	queueName                   string
	durable                     bool
	occurError                  error //  Record errors during initialization 
...
func (c *consumer) Received(routeKey string, callbackFunDealSmg func(receivedData string)) {
	...
	go func(key string) {
		...
		err = ch.QueueBind(
			queue.Name,
			key, //  Topics  pattern , The producer will deliver the message to the switch route_key，  Consumers match different key Get message 、 handle 
			c.exchangeName,
			false,
			nil,
		)
